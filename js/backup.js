/* Momentjes — backup export & import.
   Formaat: één zip met manifest.json + audio/ bestanden.
   Zo raak je nooit iets kwijt, ook niet als je telefoon zoekraakt. */
...
const Backup = (() => {
  ...
  async function buildExport() {
    const [memories, children, categories] = await Promise.all([
      DB.getAll('memories'), DB.getAll('children'), DB.getAll('categories'),
    ]);
  ...
  /* Exporteren: op iPhone via het deelmenu (Bestanden/iCloud/AirDrop/mail),
     anders als download. */
  ...
  /* Importeren: voegt samen — bestaande momentjes (zelfde id) blijven staan,
     nieuwe komen erbij. Veilig om vaker te doen. */
  async function importBackup(arrayBuffer) {
    ...
    // Namen matchen zodat hetzelfde kind uit een backup samenvalt met het bestaande profiel
    const childByName = {};
    ...
    const childMap = {}; // backup-id → lokaal id
    ...
    for (const m of manifest.memories || []) {
      if (existingMemories.has(m.id)) { skipped++; continue; }
      const mem = { ...m };
      ...
      if (m.audioFile && files[m.audioFile]) {
        ...
        // kopie van de bytes, los van de grote zip-buffer
